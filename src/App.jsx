...
const LOOKBACK_OPTIONS = [
  {
    value:   '100',
    label:   'Current Regime (100 days)',
    desc:    'Calibrates μ and σ from the last ~5 months only. High recency bias — captures what the market is doing right now. Useful for short-term shock modelling.',
...
// ── Model options ─────────────────────────────────────────────────────────────
const MODEL_OPTIONS = [
  { value: 'gbm',  label: 'Plain GBM',
    desc: 'Geometric Brownian Motion — constant vol, normally distributed returns. Textbook baseline.' },
  { value: 'jump', label: 'GBM + Jumps',
    desc: 'Merton (1976) — Poisson-distributed crash/rally events. Generates fat tails. Good for stress testing.' },
  { value: 'sv',   label: 'Stochastic Vol',
    desc: 'Heston model — vol is random, mean-reverting, negatively correlated with price (leverage effect).' },
  { value: 'full', label: '✦ Full Model',
    desc: 'Heston SV + Merton Jumps. Most realistic. Captures vol clustering, fat tails, crashes, leverage effect.' },
];

// ── Default Heston/Merton params (overridden by live calibration) ─────────────
const DEFAULT_PARAMS = {
  mu: 0.10, sigma0: 0.18,
  // Heston
  kappa: 4.0,   // mean-reversion speed
  theta: 0.0324,// long-run variance (≈ 0.18² = 0.0324)
  xi:    0.35,  // vol-of-vol
  rho:   -0.70, // leverage correlation (negative: price falls → vol rises)
  // Merton
  jumpLambda: 5,     // expected jumps per year
  jumpMu:     -0.025,// mean log-jump (negative = crash bias)
  jumpSigma:   0.05, // jump size dispersion
};
...
function pathColor(i) {
  return `hsl(${(i * 137.508) % 360},${68 + (i % 3) * 9}%,${50 + (i % 5) * 5}%)`;
}
...
function makeScalers(canvas, range, days, log) {
  ...
  const logMin = Math.log(Math.max(range.min, 1));
  const logMax = Math.log(range.max);
  const toX = d => PAD.left + (d / days) * iW;
  const toY = p => {
    const frac = log
      ? (Math.log(Math.max(p, 0.01)) - logMin) / (logMax - logMin)
      : (p - range.min) / (range.max - range.min);
...
function initCanvas(canvas, range, days, startPrice, log) {
  ...
  for (let i = 0; i <= 6; i++) {
    const frac = i / 6;
    const price = log
      ? Math.exp(Math.log(Math.max(range.min, 1)) + (1 - frac) * (Math.log(range.max) - Math.log(Math.max(range.min, 1))))
      : range.min + (1 - frac) * (range.max - range.min);
    const y = PAD.top + frac * iH;
    ctx.strokeStyle = GRID_C; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(PAD.left, y); ctx.lineTo(W - PAD.right, y); ctx.stroke();
    ctx.fillStyle = AXIS_C;
    ctx.fillText(`$${(price / 1000).toFixed(1)}k`, PAD.left - 5, y + 3);
  }

  // X grid + labels
  const xLabels = days <= 63  ? ['Now','2w','1mo','6w','2mo','10w','3mo']
                : days <= 252 ? ['Now','2mo','4mo','6mo','8mo','10mo','1yr']
                :               ['Now','4mo','8mo','1yr','16mo','20mo','2yr'];
  ...
  for (let i = 0; i <= 6; i++) {
    ...
    ctx.fillStyle = AXIS_C;
    ctx.fillText(xLabels[i] || '', x, H - PAD.bottom + 16);
  }
  ...
  ctx.setLineDash([]);
  ctx.fillStyle = '#ffffff30'; ctx.textAlign = 'right'; ctx.font = '9px IBM Plex Mono, monospace';
  ctx.fillText('Today', W - PAD.right - 3, sy - 4);
}
...
function drawPaths(canvas, paths, from, to, range, days, log) {
  ...
  for (let s = from; s < to; s++) {
    ctx.strokeStyle = pathColor(s);
    ctx.beginPath();
    ctx.moveTo(toX(0), toY(paths[s][0]));
...
export default function App() {
  ...
  function startAnimation(paths, liveStart) {
    const animate = () => {
      ...
      const to   = Math.min(from + PATHS_PER_FRAME, paths.length);
  ...
  async function handleRun(demo) {
    ...
    if (demo) {
      ...
      simParams = { ...params, ...demoCalib };
      liveStart = 5540;
      setStartPrice(liveStart);
      setCalibration({
        mu:     demoCalib.mu,
        sigma:  demoCalib.sigma0,
        source: 'Demo — S&P 500 long-run historical averages (μ≈10%, σ≈18%)',
        sessions:  null,
        oldestDate: null,
        newestDate: null,
        lookbackLabel: 'N/A',
      ...
      let fetchResult;
      try {
        fetchResult = await fetchLiveData();
      } catch (e) {
        setError(e.message);
        setStatus('idle');
        return;
      ...
      setParams(p => ({
        ...p,
        mu:         +calib.mu.toFixed(4),
        sigma0:     +calib.sigma0.toFixed(4),
        theta:      +calib.theta.toFixed(5),
        jumpLambda: +calib.jumpLambda.toFixed(1),
      }));
      ...
      setCalibration({
        mu:           calib.mu,
        sigma:        calib.sigma0,
        source:       fetchResult.meta.source,
        sessions:     fetchResult.meta.sessionsReturned,
        oldestDate:   fetchResult.meta.oldestDate,
        newestDate:   fetchResult.meta.newestDate,
        lookbackLabel: lbInfo?.label || lookback,
      });
    }
    ...
    spRef.current = liveStart;
    initCanvas(canvasRef.current, rangeRef.current, forecastDays, liveStart, logScale);
    setStatus('animating');
    startAnimation(paths, liveStart);
  }
  ...
  const horizLabel = forecastDays === 21  ? '1 Month'
    : forecastDays === 63  ? '3 Months'
    : forecastDays === 126 ? '6 Months'
    : forecastDays === 252 ? '1 Year'
    : forecastDays === 504 ? '2 Years'
    : `${forecastDays}d`;

  // ─── Render ───────────────────────────────────────────────────────────────
  return (
    <div style={{ background: BG, minHeight: '100vh', color: '#e2e8f0',
      fontFamily: "'IBM Plex Mono', monospace", padding: '18px 22px' }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type=text], input[type=number] {
          ...
          padding: 7px 10px; color: #e2e8f0; font-family: inherit; font-size: 11px;
          outline: none; width: 100%; transition: border-color .15s;
        }
        input[type=text]:focus, input[type=number]:focus { border-color: #00d4ff; }
        select {
          background: ${BG}; border: 1px solid ${BORDER}; border-radius: 6px;
          padding: 8px 10px; color: #e2e8f0; font-family: inherit;
          font-size: 11px; outline: none; cursor: pointer; width: 100%;
        }
        ...
        .btn  { border: none; border-radius: 8px; padding: 9px 20px; font-family: inherit;
        ...
        .pbar  { height: 2px; background: ${BORDER}; border-radius: 1px; overflow: hidden; }
        .pfill { height: 100%; background: linear-gradient(90deg,#0066ff,#00d4ff); transition: width .07s; }
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
          ...
          <span className="regime-tag" style={{
            background: selectedLB.color + '22',
            color: selectedLB.color,
            border: `1px solid ${selectedLB.color}44`,
          }}>
            {selectedLB.tagLine}
          </span>
        </div>
        ...
        {status === 'animating' && (
          <div className="pbar" style={{ width: 200, marginTop: 5 }}>
        ...
        {calibration && (
          <div className="fade-in" style={{ marginTop: 8, fontSize: 9, lineHeight: 1.9,
            ...
            borderRadius: 6, border: `1px solid ${BORDER}`, display: 'inline-block' }}>
            <span style={{ color: '#4a6fa5' }}>CALIBRATION · </span>
            <span style={{ color: selectedLB.color }}>{calibration.lookbackLabel}</span>
            {calibration.sessions && (
              <> · <span style={{ color: '#4a6fa5' }}>{calibration.sessions} sessions</span>
               · <span>{calibration.oldestDate}</span>
               {' → '}
               <span>{calibration.newestDate}</span></>
            )}
            <br />
            Annualised μ = <span style={{ color: '#22c55e' }}>{pct(calibration.mu)}</span>
            &nbsp;·&nbsp;
            Annualised σ = <span style={{ color: '#f97316' }}>{pct(calibration.sigma)}</span>
            &nbsp;·&nbsp;
            <span style={{ color: '#1e4a70' }}>{calibration.source}</span>
          </div>
        )}
        ...
        {stats && (
          <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', marginTop: 10 }}>
            {[
              { l: 'NOW',      v: fmt(startPrice),               c: '#9ca3af' },
              { l: 'BEAR 5%',  v: fmt(stats.p5),                 c: '#ef4444' },
              { l: 'MEDIAN',   v: fmt(stats.median),             c: '#00d4ff' },
              { l: 'BULL 95%', v: fmt(stats.p95),                c: '#22c55e' },
              { l: 'PROB UP',  v: `${stats.probUp}%`,            c: '#22c55e' },
              { l: 'EXP RTN',  v: `${stats.expectedReturn}%`,
                c: +stats.expectedReturn >= 0 ? '#22c55e' : '#ef4444' },
            ].map(s => (
              <div key={s.l}>
                <div style={{ fontSize: 8, color: '#2d5a8a', letterSpacing: '.1em' }}>{s.l}</div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(155px,1fr))',
          ...
          <div style={{ gridColumn: 'span 2' }}>
            <span className="lbl">
              LOOKBACK HORIZON
              <span style={{ marginLeft: 6, color: '#1e3a5f', fontStyle: 'italic', fontWeight: 400 }}>
                — changes how μ and σ are calibrated from historical data
              </span>
            </span>
              ...
              {LOOKBACK_OPTIONS.map(opt => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 10, fontWeight: 600, transition: 'all .15s', textAlign: 'center',
                    border: lookback === opt.value
                      ? `1.5px solid ${opt.color}`
                      : `1px solid ${BORDER}`,
                    background: lookback === opt.value
                      ? opt.color + '18'
                      : CARD_BG,
                    color: lookback === opt.value ? opt.color : '#4a6fa5',
            ...
            <div style={{ fontSize: 9, color: '#1e4a70', marginTop: 5, padding: '5px 8px',
          ...
          <div>
            ...
            <input type="text" value={apiKey}
              placeholder="Free key at alphavantage.co"
              onChange={e => setApiKey(e.target.value)} disabled={isRunning} />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 5,
            ...
            <input type="checkbox" checked={logScale} style={{ accentColor: '#00d4ff' }}
              onChange={e => setLogScale(e.target.checked)} />
            Log scale
          </label>
          <span style={{ fontSize: 10, color: '#4a6fa5', cursor: 'pointer', userSelect: 'none' }}
            onClick={() => setShowAdv(s => !s)}>
            ⚙ Advanced params {showAdv ? '▲' : '▼'}
          </span>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
            <button className="btn" disabled={isRunning} onClick={() => handleRun(true)}
              style={{ background: '#0d1f38', border: `1px solid ${BORDER}`, color: '#00d4ff' }}>
              ◈ Demo
            </button>
            <button className="btn" disabled={isRunning} onClick={() => handleRun(false)}
              style={{ background: 'linear-gradient(135deg,#0055cc,#00d4ff)', color: '#fff' }}>
              {isRunning ? '● Running...' : '▶ Live Run'}
            </button>
          </div>
        </div>
        ...
        {showAdv && (
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${BORDER}`,
            ...
            <div style={{ gridColumn: '1/-1', fontSize: 9, color: '#1e4a70', marginBottom: 4 }}>
              ⚠ When "Live Run" is used, μ, σ, θ, and λ are overridden by live calibration
              unless you run Demo Mode. Edit here to experiment with manual assumptions.
            </div>
            {[
              { k: 'mu',         l: 'DRIFT μ (ann.)',   s: 0.01, mn: -0.3, mx: 0.5  },
              { k: 'sigma0',     l: 'INIT VOL σ',       s: 0.01, mn: 0.05, mx: 1.0  },
              { k: 'jumpLambda', l: 'JUMPS/YR λ',       s: 0.5,  mn: 0,    mx: 20   },
              { k: 'jumpMu',     l: 'JUMP MEAN μⱼ',    s: 0.01, mn: -0.3, mx: 0.2  },
              { k: 'jumpSigma',  l: 'JUMP STD σⱼ',     s: 0.01, mn: 0.01, mx: 0.4  },
              { k: 'kappa',      l: 'MEAN-REV κ',       s: 0.5,  mn: 0.1,  mx: 20   },
              { k: 'theta',      l: 'LONG-RUN VAR θ',  s: 0.005,mn: 0.005,mx: 0.5  },
              { k: 'xi',         l: 'VOL-OF-VOL ξ',    s: 0.05, mn: 0.05, mx: 2.0  },
              { k: 'rho',        l: 'LEVERAGE ρ',       s: 0.05, mn: -1,   mx: 0    },
            ].map(p => (
              <div key={p.k}>
                <span className="lbl">{p.l}: <span style={{ color: '#00d4ff' }}>{params[p.k]}</span></span>
                <input type="number" value={params[p.k]} step={p.s} min={p.mn} max={p.mx}
                  disabled={isRunning}
                  onChange={e => setParams(prev => ({ ...prev, [p.k]: +e.target.value }))} />
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 12, alignItems: 'start' }}>
        ...
        <div className="card" style={{ padding: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between',
            ...
            <span>SIMULATED PRICE PATHS · {numSims.toLocaleString()} paths</span>
            {status !== 'idle' && (
              <span>{completed.toLocaleString()} / {numSims.toLocaleString()} rendered</span>
            )}
          </div>
          <div style={{ position: 'relative' }}>
            ...
            {status === 'idle' && (
              <div style={{ position: 'absolute', inset: 0, display: 'flex',
                ...
                pointerEvents: 'none' }}>
                <div style={{ fontSize: 34, color: '#1e3a5f', marginBottom: 8 }}>◈</div>
                <div style={{ fontSize: 11, color: '#1e3a5f', letterSpacing: '.2em' }}>
                  AWAITING SIMULATION
                </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div className="card">
            <div style={{ fontSize: 9, color: '#2d5a8a', marginBottom: 6,
              ...
              <span>FINAL PRICE DISTRIBUTION</span>
              {status !== 'idle' && (
                <span style={{ color: status === 'done' ? '#22c55e' : '#00d4ff' }}>
                  {completed}/{numSims}
                </span>
              )}
            </div>
            ...
            {histData.length > 0 ? (
              <ResponsiveContainer width="100%" height={195}>
                <ComposedChart data={histData} margin={{ top: 4, right: 4, left: -26, bottom: 4 }}>
                  ...
                  <XAxis dataKey="priceLabel" stroke={BORDER} tick={{ fill: AXIS_C, fontSize: 8 }}
                    interval={Math.floor(histData.length / 5)} />
                  <YAxis stroke={BORDER} tick={{ fill: AXIS_C, fontSize: 8 }} />
                  <Tooltip
                    contentStyle={{ background: CARD_BG, border: `1px solid ${BORDER}`,
                      borderRadius: 6, fontSize: 10 }}
                    formatter={(v, n) => [v, n === 'count' ? 'Simulations' : 'Normal fit']}
                    labelFormatter={l => `~${l}`}
                  />
                  <Bar dataKey="count" radius={[1, 1, 0, 0]} isAnimationActive={false}>
                    {histData.map((d, i) => (
                      <Cell key={i}
                        fill={d.above ? '#14532d' : '#7f1d1d'}
              ...
              <div style={{ height: 195, display: 'flex', alignItems: 'center',
                justifyContent: 'center', fontSize: 10, color: '#1e3a5f' }}>
                Updates live as paths render...
              </div>
            )}

            <div style={{ display: 'flex', gap: 10, marginTop: 6, fontSize: 9, color: '#4a6fa5' }}>
              <span>■ <span style={{ color: '#166534' }}>Above today</span></span>
              <span>■ <span style={{ color: '#7f1d1d' }}>Below today</span></span>
              <span>— <span style={{ color: '#fbbf24' }}>Normal fit</span></span>
            </div>
          </div>
          ...
          {stats && (
            <div className="card fade-in">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '7px 12px', marginBottom: 10 }}>
                {[
                  { l: 'Expected Return',  v: `${stats.expectedReturn}%`,
                    c: +stats.expectedReturn >= 0 ? '#22c55e' : '#ef4444' },
                  { l: 'Prob. Higher',     v: `${stats.probUp}%`,       c: '#22c55e' },
                  { l: 'Prob. > +20%',     v: `${stats.probUp20}%`,     c: '#fbbf24' },
                  { l: 'Prob. < −20%',     v: `${stats.probDn20}%`,     c: '#ef4444' },
                  { l: 'Std Dev',          v: fmt(stats.std),           c: '#a78bfa' },
                  { l: 'Excess Kurtosis',  v: stats.excessKurtosis,
                    c: +stats.excessKurtosis > 0.5 ? '#f97316' : '#9ca3af' },
                ].map(s => (
                  <div key={s.l}>
                    <div style={{ fontSize: 8, color: AXIS_C }}>{s.l}</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)',
                ...
                {[
                  { l: '5th',  v: fmt(stats.p5),     c: '#ef4444' },
                  { l: '25th', v: fmt(stats.p25),    c: '#f97316' },
                  { l: '50th', v: fmt(stats.median), c: '#fff'    },
                  { l: '75th', v: fmt(stats.p75),    c: '#00d4ff' },
                  { l: '95th', v: fmt(stats.p95),    c: '#22c55e' },
                  { l: 'Now',  v: fmt(startPrice),   c: '#6b7280' },
                ].map(s => (
                  <div key={s.l} style={{ padding: '5px 2px', background: '#070b14', borderRadius: 4 }}>
                    <div style={{ color: '#2d5a8a' }}>{s.l}</div>
                    <div style={{ color: s.c, fontWeight: 600, fontSize: 11 }}>{s.v}</div>
                  </div>
                ))}
              </div>

              {+stats.excessKurtosis > 0.5 && (
                <div style={{ marginTop: 10, padding: '7px 9px', background: '#1a1000',
                  border: '1px solid #92400e', borderRadius: 6, fontSize: 9,
                  color: '#fbbf24', lineHeight: 1.6 }}>
                  ⚠ Excess kurtosis {stats.excessKurtosis} → fat tails detected.
                  More extreme outcomes than a normal distribution predicts — consistent with
                  real S&P 500 behaviour.
                </div>
              )}
            </div>
          )}
          ...
          {status === 'done' && (
            <div className="card fade-in" style={{ fontSize: 9, color: '#2d5a8a', lineHeight: 1.8 }}>
              ...
              Run the same model again with a different lookback to see how calibration
              window changes μ and σ — and therefore the output distribution.
              A 100-day window captures today's regime. A 5-year window reflects full-cycle
              structural drift. Both are valid framings of different questions.
            </div>
          )}
        </div>
      </div>

      <div style={{ marginTop: 12, fontSize: 8, color: '#1e3a5f', fontStyle: 'italic', lineHeight: 1.7 }}>
        Not financial advice. GBM-family models assume log-normal returns and constant parameters
        between rebalancing. Regime shifts, central bank intervention, liquidity crises, and
        correlation breakdown in tail events are not captured. Use as a probabilistic framework,
